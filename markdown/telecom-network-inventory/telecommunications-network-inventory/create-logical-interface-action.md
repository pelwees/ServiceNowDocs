...
Parent network interface record to create the relationship with a logical interface. Click the add icon \(![Add icon](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/telecom-network-inventory/telecommunications-network-inventory/../image/add-icon-1.png)\) to add an interface.
