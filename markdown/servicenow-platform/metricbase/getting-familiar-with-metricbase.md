...
1.  Navigate to **MetricBase Demo** &gt; **Drones** to see the name, model, length, width, altitude, and speed for all drones in the fleet. ![Drone metrics](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/drone-metrics.png)
2.  Select **MetricBase Demo** &gt; **Metrics** to see how the metrics are defined. In the Time Series Metrics list, the column values specify the table where the drone's time-series metrics are stored, the names of all the metrics that are stored for each drone, and the retention policy for each metric. The retention policy specifies how frequently measurements are taken, and how long MetricBase retains that data.
    ...
    ![Metric definitions](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/metric-definitions.png)
...
3.  Select a retention policy. The Retention Policy form shows the definition for the policy. In the following example, this metric is measured once a minute for the first 8 days, then once every 10 minutes for the following 94 days, and then once an hour for the following 397 days. After that, the system discards the data.
    ...
    ![Retention policy](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/retention-policy.png)

    See [MetricBase retention policies](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/metricbase-retention-policies.md).
...
5.  Select a table name. The Time Series Metric definition shows the **Retention policy aggregator**. This aggregator summarizes the data as, for example, the average, maximum, or minimum value in the aggregation period. In the following example, the admin clicked the table for the speed metric. The result is that MetricBase stores the speed averages during each sampling period in the MetricBase database.
    ...
    ![Data aggregation](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/data-aggregation.png)
...
9.  Select the speed value of the first drone. The following graph shows the speed of the drone that was measured hourly.
    ...
    ![Speed measured over time](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/speed-measured-hourly.png)
...
11. Select the menu icon next to the Altitude column heading and select **Time Series Chart**. You see a graph that shows the altitude measurements of the Warbler-T7-type drones.
    ...
    ![Altitude of all the drones](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/altitude-drones.png)
...
13. Click the menu icon next to the Altitude column heading and select **Time Series Chart Designer** to see another way of visualizing the data. ![Create a report](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/create-a-report.png)
14. In the Create a Report page that displays in the Reporting application, vary the graph parameters.
    ...
    **Note:** In the following steps, to stay on the same page, select **Run**. To advance to the next page when designing the graph, select **Next**.
    ...
    1.  In the Type pane, select the kind of graph to display, and then click **Next**. ![Graph type](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/type-of-graph.png)
    ...
    3.  In the Style pane, change the **Chart color** value, and then click **Run**. ![Style pane](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/style-pane.png)
...
15. Select **MetricBase Demo** &gt; **Trigger Definitions**. The MetricBase Trigger Definitions list shows the name \(**Name**\) of the trigger, the metric \(**Metric**\) the trigger monitors, the table \(**Table name**\) the metric that it is in, the trigger description, and whether the trigger is active. Triggers kick off flows when metrics meet trigger thresholds. An example is when a drone flies too low. For more information about triggers, see [Trigger Workflows](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../concept/metricbase-triggers.md).![Trigger definitions](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/trigger-definitions.png)
16. Select **Low-Battery Drone**. The MetricBase Linear Predictor trigger form displays the definition of the Low-Battery Drone trigger.![Linear trigger](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/trigger-example-low-altitude.png)
...
19. In the **Flow name** field, name the flow and select **Submit**. ![Flow Properties dialog box](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/flow-dialog-box.png)
20. Click **Add a Trigger** and select **MetricBase** in the list. ![Flow definition page](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/flow-definition.png)
21. In **MetricBase Trigger**, select a trigger that already exists and select **Done**. Alternately, select the plus sign to the right of **MetricBase Trigger** to create a new trigger. ![Choose a MetricBase trigger](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/flow-mb-trigger-selection.png)
...
23. Select **Action** to specify the actions to take when the trigger fires. The Action dialog box appears.
    ...
    ![Define an action](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../image/flow-action.png)
...
|Data Explorer|Widget that enables you to experiment with MetricBase JavaScript APIs using the demonstration application data. See [Get Familiar with MetricBase APIs](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../task/metricbase-data-explorer.md) for more information.|

**Parent Topic:**[Working with MetricBase demo data](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/servicenow-platform/metricbase/../concept/working-with-metricbase-demo-data.md)
