---
title: Subscriptions
description: See a list of your subscriptions for all of your instances and the list of applications that you’re entitled to, as part of those subscriptions, in one comprehensive view.
locale: en-US
release: australia
product: Impact Product Adoption Roadmap
classification: impact-product-adoption-roadmap
topic_type: concept
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Product Adoption, Using Impact, Impact]
---

# Subscriptions

See a list of your subscriptions for all of your instances and the list of applications that you’re entitled to, as part of those subscriptions, in one comprehensive view.

Subscriptions display a list of all ServiceNow subscriptions and the amount purchased that your organization is entitled to.

![Shows the subscriptions for all of your instances.](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/impact/impact-product-adoption-roadmap/../image/subscriptions.png)

-   For each instance, sort the list by alphabetical order, type of subscription meter, by the total amount purchased, or by subscription start and end date.
-   If there are secondary meters, a chevron is available to the left of the subscription that can be selected to display those meters.

    ![Shows subscriptions with a secondary meter expanded.](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/impact/impact-product-adoption-roadmap/../image/subscriptions-2nd-meter.png)

-   Select each subscription to see the capabilities that are included in the subscription bundle.

    ![Shows the expanded list of capabilities.](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/impact/impact-product-adoption-roadmap/../image/subscriptions-list.png)

-   Select each application to access the description.

    ![Shows the application description fly out from the Subscriptions list.](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/impact/impact-product-adoption-roadmap/../image/subscriptions-descriptions.png)


**Note:** To allocate subscriptions, log in to your instance and navigate to **Subscription Management**.

