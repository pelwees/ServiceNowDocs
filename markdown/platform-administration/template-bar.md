---
title: Template bar
description: Use the template bar to apply, edit, and create templates.
locale: en-US
release: australia
topic_type: concept
last_updated: "2026-03-12"
reading_time_minutes: 1
breadcrumb: [Form templates, Administer, Form administration, Forms, fields, and lists, Configure core features, Administer the ServiceNow AI Platform]
---

# Template bar

Use the template bar to apply, edit, and create templates.

When toggled, the template bar appears at the bottom of the form. Select a template title to apply the template to the form. Use the buttons at the bottom-right to add, edit, and access the complete list of templates.

![Template Bar Elements](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/../image/templateBar.png "Template Bar in Core UI")

|Element|Description|
|-------|-----------|
|Templates|Available templates appear in blue text on the template bar. Click a template to apply it to the form.|
|All Templates button \(![All Templates button](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/../image/templateBarAllTemplates.png)\)|Toggles the All Templates window.|
|Create New Template button \(![Create New Template button](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/../image/templateBarNewTemplate.png)\)|Displays the Create New Template window.|
|All Templates popup|This window displays when the **All Templates** button is selected. The window displays a filterable list of all available templates. Click the **Edit** button to the left of any template to edit that template.|
|Disable Template Bar button \(![Disable Template Bar button](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/../image/templateBarDisable.png)\)|Disables the template bar. To display the bar, see [Toggle the template bar](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/../task/t_ToggleTheTemplateBar.md)|

For non-admins, see [KB0711956](https://support.servicenow.com/kb?id=kb_article_view&sysparm_article=KB0711956) for more information on enabling the Edit button.

**Parent Topic:**[Using form templates](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/c_Templates.md)

**Related topics**  


[Create a template using the Template form](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/../task/t_CreateATemplateUsingTheTmplForm.md)

[Create templates for related task records](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/../task/create-templates-for-related-task-records.md)

[Create a template by saving a form](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/../task/t_SaveAFormAsATemplateInUI16OrUI15.md)

[Schedule record creation from a template](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/../task/t_ScheduleATemplate.md)

[Create a module for a template](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/../task/t_CreateAModuleForATemplate.md)

[Toggle the template bar](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/../task/t_ToggleTheTemplateBar.md)

[Scripted templates](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-administration/../reference/r_ScriptedTemplates.md)

