...
User name that the proxy uses to log in to the instance. The user must have the edge\_encryption role. See [Set up an Edge Encryption user account](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/platform-security/edge-encryption/../task/t_SetupEdgeEncryptionUser.md).
