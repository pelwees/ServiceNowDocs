...
The message that appears at the top of the recommendation card when the guidance action is complete. For example, **KB article was shared**.
    ...
    -   Configure outputs in the **Guidance Outputs** tab.
        ...
        These fields can contain static text, dynamic content \(field values from guidance inputs\), or a combination of static text and dynamic content. To use dynamic content, select the Pill-picker icon \(![Pill-picker icon](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/customer-service-management/../image/icon-pill-picker.png)\) next to the field and select a guidance input from the list.
...
Configure a guidance node and link the created guidance to this node to [provide actions to agents](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/customer-service-management/configure-guidance-in-gdb.md).
