...
2.  To create an object that determines the badge properties, open the page's **Client state parameters** dialog.
    ...
    ![Pointer hovering over selected Client state parameters and the Edit client state parameters dialog as it first appears](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/application-development/ui-builder/../image/badge-edit-clientstateparams.png)

    1.  Replace the **Name** with `badgeConfigState`, and from the **Type** dropdown, choose **JSON**.

        ![Edit client state parameters dialog with the Name changed to badgeConfigState and JSON being selected from the Type dropdown](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/application-development/ui-builder/../image/badge-edit-02clientstateparams.png)
    ...
    4.  Select **Add property** and add the badge **color**, **label**, and **variant** properties.
        ...
        ![Initial Value pop up with 3 properties added, color, label, and variant](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/application-development/ui-builder/../image/badge-client-state01.png)

    5.  Select **Apply** to return to the **Edit client state parameters** dialog.

        ![Returned to Edit client state parameters with badgeConfigState JSON parameters](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/application-development/ui-builder/../image/badge-client-state02.png)
...
3.  To set the badge config to the object just created, in the **Content** tree, select the **Carousel** component.
    ...
    1.  On the **Config** tab for your Carousel component, hover over the **Badge configuration** property and select the **Bind data or use scripts** icon.
        ...
        ![Carousel Badge configuration showing the 3 parameters added and the pointer hovering over the Bind data or use scripts button](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/application-development/ui-builder/../image/badge-bind-data-badgeconfig.png)
    ...
    3.  Select **badgeConfigState \(3\)**, and to add it to the data output area, select the up arrow icon.
        ...
        ![Bind data to Badge configuration dialog with Client states selected in Data types and badgeConfigState 3 highlighted with pointer hovering over up arrow button](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/application-development/ui-builder/../image/badge-02bind-data-badgeconfig.png)

    4.  Select **Apply**.

        The **badgeConfigState** parameter should be added to the Carousel **Badge configuration**.

    ![Carousel Configure tab with the Badge configuration field displaying badgeConfigState in addition to the 3 previously added parameters](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/application-development/ui-builder/../image/badge-config-final.png)

4.  Open the **Edit client script** dialog by selecting **Add a new** **Edit client script**.

    ![Pointer hovering over Add new plus sign button and Edit client script dialog that appears when Add new button is selected](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/application-development/ui-builder/../image/badge-edit-clientscript01.png)
...
5.  To create a client script that updates the **badgeClientState "label"** parameter value to the number of items in the Carousel, enter: `api.setState(`badgeConfigState`, {...api.state.badgeConfigState, "label": `count: ${api.data.look_up_multiple_records_1.results.length}`});`
    ...
    ![Edit client script dialog with client script added](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/application-development/ui-builder/../image/badge-edit-clientscript04.png)
...
6.  Add an event to the data resource so that your client script triggers whenever the data resource is updated.
    ...
    1.  In the bottom left, select the data resource **Look up multiple records 1** &gt; **Events tab**.
        ...
        ![Pointer, in the Data and scripts section, hovering over the Look up multiple records 1 data resource and Edit Look up multiple records 1 dialog Events tab selected](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/application-development/ui-builder/../image/badge-edit-dataresource01.png)
    ...
    3.  Select **Add handler**, scroll down, and select the new client script you created, and select **Continue**.
        ...
        ![Dialog for adding the New client script 1 event handler](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/application-development/ui-builder/../image/badge-choose-eventhandler.png)
...
7.  In the upper right corner, select **Save** &gt; **select the Preview dropdown arrow** &gt; **Open URL path**.
    ...
    ![Final display showing the Title followed by count: 12](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/application-development/ui-builder/../image/badge-final01.png)
