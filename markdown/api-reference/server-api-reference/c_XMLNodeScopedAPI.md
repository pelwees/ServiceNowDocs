...
</table>The following example shows how to parse an XML string with CDATA reporting enabled using [Scoped XMLDocument2 - setEnableCDATAReporting\(Boolean enable\)](https://raw.githubusercontent.com/pelwees/ServiceNowDocs/australia/markdown/api-reference/server-api-reference/../../XMLDocument2ScopedAPI/concept/c_XMLDocument2ScopedAPI.md#). The code uses isCDATANode\(\) to show that the first node queried in the XML string is a CDATA node.
